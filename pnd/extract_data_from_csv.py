...
def rename_columns_to_target_structure(df: pd.DataFrame) -> pd.DataFrame:
    """
    Renames columns to match the target structure for PND data.
    """
    ...
    for col in df.columns:
        ...
        elif "Zona de Carga" in clean_col:
            column_mapping[col] = "ZonaCarga"
        elif "Precio Zonal ($/MWh)" in clean_col:
            column_mapping[col] = "PrecioZonal_MWh"
        elif "Componente energia ($/MWh)" in clean_col:
            column_mapping[col] = "Energia_MWh"
        elif "Componente perdidas ($/MWh)" in clean_col:
            column_mapping[col] = "Perdidas_MWh"
        elif "Componente Congestion ($/MWh)" in clean_col:
            column_mapping[col] = "Congestion_MWh"
...
def process_csv_file(file_path: str) -> List[Dict]:
    """
    Processes a single PND CSV file and returns a list of dictionaries with the target structure.
    """
    ...
    if not sistema:
        notify_error(f"[PND] No se pudo extraer el Sistema del nombre del archivo: '{filename}'")
        return []

    if not fecha_operacion:
        notify_error(f"[PND] No se pudo extraer la FechaOperacion del nombre del archivo: '{filename}'")
        return []
    ...
    try:
        ...
        try:
            ...
            notify_error(f"[PND] Error al leer el archivo '{filename}' con encoding latin-1: {e}")
            return []
    except Exception as e:
        notify_error(f"[PND] Error inesperado al abrir el archivo '{filename}': {e}")
        return []
    ...
    header_line_idx = find_header_row(temp_df, "Hora", "Zona de Carga")
    logging.info(f"Header line index: {header_line_idx}")

    if header_line_idx == -1:
        notify_error(f"[PND] No se encontro la linea de encabezado en el archivo: '{filename}'")
        return []
    ...
    try:
        ...
        notify_error(f"[PND] Error al parsear CSV '{filename}': {e}")
        return []
    ...
    for _, row in df.iterrows():
        ...
        try:
            record = {
                ...
                "ZonaCarga": str(row.get("ZonaCarga", "")).strip().replace('"', ""),
                "Sistema": sistema,
                "PrecioZonal_MWh": float(
                    str(row.get("PrecioZonal_MWh", 0)).replace(",", "")
                ),
                "Energia_MWh": float(str(row.get("Energia_MWh", 0)).replace(",", "")),
                "Perdidas_MWh": float(str(row.get("Perdidas_MWh", 0)).replace(",", "")),
                "Congestion_MWh": float(
                    str(row.get("Congestion_MWh", 0)).replace(",", "")
                ),
            }
