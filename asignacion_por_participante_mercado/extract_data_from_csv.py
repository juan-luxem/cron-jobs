    ...
    for _, row in df.iterrows():
        ...
        try:
            record = {
                ...
                'Sistema': sistema,
                'CantidadTotalEnergiaAsignada_MWh': float(str(row.get('CantidadTotalEnergiaAsignada_MWh', 0)).replace(',', '')),
                'ReservaRegulacion_MW': float(str(row.get('ReservaRegulacion_MW', 0)).replace(',', '')),
