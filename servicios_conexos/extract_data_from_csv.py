...
def rename_columns_to_target_structure(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        elif "Zona de reserva" in clean_col:
            column_mapping[col] = "ZonaReserva"
        elif "Tipo de reserva" in clean_col:
            column_mapping[col] = "TipoReserva"
        elif "Precio de la reserva" in clean_col:
            column_mapping[col] = "PrecioReserva_MW_Hora"
...
def process_csv_file(file_path: str) -> List[Dict]:
    ...
    header_line_idx = find_header_row(temp_df, "Hora", "Zona de reserva")
    ...
    for _, row in df.iterrows():
        ...
        try:
            record = {
                ...
                "TipoReserva": str(row.get("TipoReserva", "")).strip().replace('"', ""),
                "PrecioReserva_MW_Hora": float(
                    str(row.get("PrecioReserva_MW_Hora", 0)).replace(",", "")
                ),
            }
    ...
    # Remove duplicates - keep the first occurrence of each (HoraOperacion, ZonaReserva, TipoReserva) combination
    # This handles cases where hour 2 appears twice and the first one is the correct data
    initial_count = len(result)
    seen_keys = {}

    for record in result:
        key = (record["HoraOperacion"], record["ZonaReserva"], record["TipoReserva"])
        # Only keep the first occurrence
        if key not in seen_keys:
            seen_keys[key] = record

    # Convert back to list, sorted by ZonaReserva, TipoReserva and HoraOperacion for consistency
    deduplicated_result = sorted(
        seen_keys.values(),
        key=lambda x: (x["ZonaReserva"], x["TipoReserva"], x["HoraOperacion"]),
    )
