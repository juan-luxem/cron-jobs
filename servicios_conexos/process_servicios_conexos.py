...
def process_servicios_conexos_data(
    ...
    if market_type not in ["MDA", "MTR"]:
        notify_error(f"[Servicios Conexos] Tipo de mercado invalido: '{market_type}'. Se esperaba 'MDA' o 'MTR'.")
        return
    ...
    logging.info(f"🚀 Starting Servicios Conexos {market_type} data processing")
    ...
    if "error" in summary:
        notify_error(
            f"[Servicios Conexos {market_type}] Error al procesar archivos CSV. "
        ...
        if summary["processed"] == summary["total"] and summary["remaining"] == 0:
            ...
            notify_error(
                f"[Servicios Conexos {market_type}] Fallo al procesar {summary['failed']} de {summary['total']} archivos. "
                f"Rango: {date_range_info}. "
