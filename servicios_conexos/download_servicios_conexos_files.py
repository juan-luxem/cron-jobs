...
from servicios_conexos.constants import ID_REPORT, MIN_DATE, URL, VIEW_GENERATOR


def download_servicios_conexos_files(
    market_type: str,
    ...
    if start_date and end_date:
        try:
            ...
            notify_error(f"Date parsing error in Servicios Conexos {market_type}: {e}")
            return
    else:
...
def _process_single_range(
    ...
    try:
        ...
        notify_error(
            f"Error initializing session for Servicios Conexos {market_type}: {e}"
        )
        return
    ...
    try:
        ...
        notify_error(
            f"Error extracting initial values for Servicios Conexos {market_type}: {e}"
        )
        return
    ...
    if fmt_start and fmt_end:
        ...
        logging.info(f"Downloading Servicios Conexos range: {period_str}{sistema_msg}")
    else:
        ...
        sistema_msg = f" for {sistema}" if sistema else ""
        logging.info(
            f"Downloading Servicios Conexos default/current period{sistema_msg}."
        )
    ...
    try:
        ...
        notify_error(f"Error extracting dates for Servicios Conexos {market_type}: {e}")
        return
    ...
    for current_sistema in sistemas_to_process:
        ...
        try:
            ...
            notify_error(
                f"Error switching system to {current_sistema} in Servicios Conexos {market_type}: {e}"
            )
            continue
        ...
        try:
            ...
            notify_error(
                f"Error setting period for {current_sistema} in Servicios Conexos {market_type}: {e}"
            )
            continue
        ...
        try:
            ...
            if r_down.status_code == 200:
                ...
                notify_error(
                    f"Download failed for {current_sistema} in Servicios Conexos {market_type}: HTTP {r_down.status_code}"
                )
        except Exception as e:
            notify_error(
                f"Error downloading {current_sistema} in Servicios Conexos {market_type}: {e}"
            )
