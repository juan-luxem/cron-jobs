...
def process_salidas_adelanto():
    ...
    API_ENDPOINT = (
        f"{API_URL}api/v1/salidas-forzadas/bulk/insert?batch_size=1000&validate=true"
    )
    ...
    if "error" in summary:
        ...
        send_telegram_message(
            bot_token, chat_id, f"Error en process_salidas_adelanto: {summary['error']}"
        )
    else:
        ...
        if summary["processed"] == summary["total"] and summary["remaining"] == 0:
            ...
            send_telegram_message(
                bot_token,
                chat_id,
                f"Fallo en process_salidas_adelanto: {summary['failed']} de {summary['total']} archivos.",
            )
