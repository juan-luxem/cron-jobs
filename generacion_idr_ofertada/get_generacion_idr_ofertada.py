...
from generacion_idr_ofertada.constants import (
    URL,
    ID_REPORT,
    MIN_DATE,
    VIEW_GENERATOR,
...
def get_generacion_idr_ofertada(market_type: str):
    """
    Generic function to download Ofertas de Venta – Recursos Interm Despachables data
    from CENACE for any market type using requests.
    """
    ...
    for sistema in SISTEMAS:
        ...
        try:
            # 1. Start a FRESH session per system
            ...
            # Use "D" initially, and force "SIN" to match ViewState
            # ---------------------------------------------------------
            data_report = {
            ...
            # ---------------------------------------------------------
            # STEP 2: Select System (Async)
            # Switch to Target System and change Periodicity to "H" (Hourly)
            # ---------------------------------------------------------
            data_system = {
                "ctl00$ContentPlaceHolder1$ScriptManager": "ctl00$ContentPlaceHolder1$ScriptManager|ctl00$ContentPlaceHolder1$ddlSistema",
                "ctl00$ContentPlaceHolder1$ddlReporte": ID_REPORT[market_type],
                "ctl00$ContentPlaceHolder1$ddlPeriodicidad": "H",  # Switched to H
                "ctl00$ContentPlaceHolder1$ddlSistema": sistema,  # Switched to Target
                "ctl00$ContentPlaceHolder1$txtPeriodo": period,
                "ctl00$ContentPlaceHolder1$hdfStartDateSelected": date,
                "ctl00$ContentPlaceHolder1$hdfEndDateSelected": date,
                "ctl00$ContentPlaceHolder1$hdfMinDateToSelect": MIN_DATE[market_type],
            ...
            # Use ddlPeriodicidad=H here!
            # ---------------------------------------------------------
            nodos_data = (
                f"ctl00%24ContentPlaceHolder1%24ddlReporte={id_report_encoded}&"
                f"ctl00%24ContentPlaceHolder1%24ddlPeriodicidad=H&"  # Updated to H
