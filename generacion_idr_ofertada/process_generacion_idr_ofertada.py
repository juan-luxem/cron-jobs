...
def process_generacion_idr_ofertada(market_type: str):
    """
    Processes Ofertas de Venta – Recursos Interm Despachables data for the specified market type (MDA or MTR).
    ...
    API_ENDPOINT = f"{API_URL}api/v1/generacion-idr-ofertada/bulk?market={market_type}"
    ...
    logging.info(
        f"🚀 Starting Ofertas de Venta – Recursos Interm Despachables {market_type} data processing"
    )
    ...
    if not os.path.exists(download_folder):
        ...
        send_telegram_message(
            bot_token,
            chat_id,
            f"Error en process_generacion_idr_ofertada ({market_type}): Download folder not found",
        )
        return
    ...
    if "error" in summary:
        ...
        send_telegram_message(
            bot_token,
            chat_id,
            f"Error en process_generacion_idr_ofertada ({market_type}): {summary['error']}",
        )
    else:
        ...
        if summary["processed"] == summary["total"] and summary["remaining"] == 0:
            ...
            send_telegram_message(
                bot_token,
                chat_id,
                f"Fallo en process_generacion_idr_ofertada ({market_type}): {summary['failed']} de {summary['total']} archivos.",
            )
