...
def get_capacidad_transferencia_data(csv_directory):
    ...
    if isinstance(csv_files, list):
        for file_path in csv_files:
            ...
            logging.info(
                f"Skipping {csv_skiprows} rows to find header in {os.path.basename(file_path)}"
            )
            print(
                f"Skipping {csv_skiprows} rows to find header in {os.path.basename(file_path)}"
            )
            ...
            try:
                ...
                missing_headers = [
                    h for h in header_map_stripped_keys.keys() if h not in df.columns
                ]
                if missing_headers:
                    logging.warning(
                        f"Missing expected headers in {os.path.basename(file_path)}: {missing_headers}"
                    )
                    files_failed += 1
                ...
                df_processed = df[
                    [col for col in api_columns if col in df.columns]
                ].copy()
                ...
                if "FechaOperacion" in df_processed.columns:
                    ...
                    df_processed["FechaOperacion"] = pd.to_datetime(
                        df_processed["FechaOperacion"],
                        format="%d/%m/%Y",
                        errors="coerce",
                    )
                    ...
                    if len(df_processed) < original_count:
                        logging.warning(
                            f"  WARNING: Dropped {original_count - len(df_processed)} rows due to invalid date format in FechaOperacion."
                        )
                    # Convert valid dates to 'YYYY-MM-DD' string format for JSON
                    df_processed["FechaOperacion"] = df_processed[
                        "FechaOperacion"
                    ].dt.strftime("%Y-%m-%d")
                else:
                    logging.warning(
                        f"  WARNING: 'FechaOperacion' column not found in {os.path.basename(file_path)}. Cannot process dates."
                    )
                    files_failed += 1
                ...
                numeric_cols = [
                    "Horario",
                    "CapTransDisImpComMwh",
                    "CapResImpEneInadMwh",
                    "CapResImpConfMWh",
                    "CapAbsTransDisImpMWh",
                    "CapTransDisExpComMwh",
                    "CapResExpEneInaMwh",
                    "CapResExpConfMwh",
                    "CapAbsTransDisExpMwh",
                ]
                ...
                df_processed = df_processed.astype(object).where(
                    pd.notnull(df_processed), None
                )
                ...
                if df_processed.empty:
                    logging.warning(
                        f"  WARNING: No valid data rows found in {os.path.basename(file_path)} after processing."
                    )
                    continue
                ...
                logging.info(
                    f"Prepared {len(payload_list)} rows to send to API from {os.path.basename(file_path)}."
                )

                # 8. Make the SINGLE POST request for the entire file's data
                try:
                    response = requests.post(
                        API_ENDPOINT, headers=headers, json=payload_list
                    )  # Send the list
                    ...
                    if 200 <= response.status_code < 300:
                        logging.info(
                            f"  SUCCESS: File data sent successfully (Status: {response.status_code}). Response: {response.text[:200]}..."
                        )
                        files_succeeded += 1
                    # Handle potential batch-specific errors (e.g., 422 Unprocessable Entity if some records failed validation)
                    elif response.status_code == 400:
                        logging.error(
                            f"  FAILED: Bad Request (400) - Issue with payload structure or headers? Response: {response.text[:300]}..."
                        )
                        files_failed += 1
                    elif (
                        response.status_code == 409
                    ):  # How the API handles conflicts in batch needs clarification
                        logging.warning(
                            f"  WARNING/FAILED: Conflict (409) - Some/all records might already exist. Response: {response.text[:300]}..."
                        )
                        # Decide if this counts as success or failure based on requirements
                        files_failed += 1  # Defaulting to failure for simplicity
                    elif (
                        response.status_code == 422
                    ):  # Often used for validation errors in batch requests
                        logging.error(
                            f"  FAILED: Unprocessable Entity (422) - Validation errors in batch? Response: {response.text[:300]}..."
                        )
                        files_failed += 1
                    else:
                        logging.error(
                            f"  FAILED: Unexpected status code {response.status_code}. Response: {response.text[:300]}..."
                        )
                        files_failed += 1
                    ...
                    time.sleep(
                        5
                    )  # Wait longer before processing next file on connection error
                ...
                logging.error(
                    f"  ERROR: Value Error processing file {os.path.basename(file_path)}: {e}"
                )
                files_failed += 1
            except Exception as e:
                logging.error(
                    f"  ERROR: An unexpected error occurred processing file {os.path.basename(file_path)}: {e}"
                )
                files_failed += 1
    ...
    if files_failed > 0:
        send_telegram_message(
            bot_token,
            chat_id,
            f"Fallo en el procesamiento de capacidad de transferencia: {files_failed} de {files_processed} archivos.",
        )
