...
from .constants import SISTEMAS, URLMTR, HEADERS

API_TARGET_SOURCE_PMLMTR = "pnd_mtr"
...
def get_pnd_mtr():
    ...
    for sistema in SISTEMAS:
        data = {
            ...
            "ctl00$ContentPlaceHolder1$ddlReporte": "363,326",
            ...
            "ctl00$ContentPlaceHolder1$hdfMinDateToSelect": "27/03/2017",
        ...
        if response.status_code == 200:
            ...
            if "attachment" in content_disposition and ".zip" in content_disposition:
                ...
                new_filename = f"PND_MTR_{sistema}.csv"
    ...
    try:
        ...
        if all_different:
            ...
            date_sin = extract_date("PND_MTR_SIN.csv")
            date_bca = extract_date("PND_MTR_BCA.csv")
            date_bcs = extract_date("PND_MTR_BCS.csv")
            ...
            if date_sin == date_bca == date_bcs:
                ...
                data_type_to_check = (
                    API_TARGET_SOURCE_PMLMTR  # Use the constant defined for this script
                )
                ...
                if exists is None:
                    ...
                    # Keep CSVs for potential manual check or retry? Or delete? Decide based on workflow.
                ...
                df_sin = preprocess_csv("PML_MTR_SIN.csv", "SIN")
                df_bca = preprocess_csv("PML_MTR_BCA.csv", "BCA")
                df_bcs = preprocess_csv("PML_MTR_BCS.csv", "BCS")
                ...
                for i in range(num_batches):
                    ...
                    batch_success = send_dataframe_to_api(
                        df_batch, API_URL, API_TARGET_SOURCE_PMLMTR
                    )
                    if batch_success:
