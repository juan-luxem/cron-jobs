...
from constants import SISTEMAS, URLMTR, API_BASE_URL, HEADERS

API_TARGET_SOURCE_CONEXOMTR = "pnd_mtr"
...
def get_precios_conexos_mtr():
    load_dotenv()  # Load environment variables from .env file
    bot_token = os.getenv("TELEGRAM_BOT_MERCADOS_LUX_TOKEN")
    chat_id = os.getenv("TELEGRAM_CHAT_ID")

    session = requests.session()
    response = session.get(URLMTR, headers={"User-Agent": "Mozilla/5.0"})
    ...
    for sistema in SISTEMAS:
        data = {
            ...
            "ctl00$ContentPlaceHolder1$ddlReporte": "364,327",
            ...
            "ctl00$ContentPlaceHolder1$hdfMinDateToSelect": "27/01/2017",
            "ctl00$ContentPlaceHolder1$hdfMaxDateToSelect": date,
            "__EVENTTARGET": "ctl00$ContentPlaceHolder1$ddlReporte",
            "__EVENTARGUMENT": "",
            "__LASTFOCUS": "",
            "__VIEWSTATE": view_state,
            "__VIEWSTATEGENERATOR": "94619A0E",
            "__VIEWSTATEENCRYPTED": "",
            "__ASYNCPOST": "true",
            "": "",
        }

        response = session.post(URLMTR, headers=HEADERS, data=data)
        soup = BeautifulSoup(response.text, "html.parser")
        ...
        response = session.post(URLMTR, headers=HEADERS, data=body)
        soup = BeautifulSoup(response.text, "html.parser")
        # Regular expression to capture the VIEWSTATE value
        match = re.search(r"\|hiddenField\|__VIEWSTATE\|([^|]+)", response.text)
        ...
        conexo_data = f"ctl00%24ContentPlaceHolder1%24ddlReporte=364%2C327&ctl00%24ContentPlaceHolder1%24ddlPeriodicidad=D&ctl00%24ContentPlaceHolder1%24ddlSistema={sistema}&ctl00%24ContentPlaceHolder1%24txtPeriodo={period_encoded}&ctl00%24ContentPlaceHolder1%24hdfStartDateSelected={date_encoded}&ctl00%24ContentPlaceHolder1%24hdfEndDateSelected={date_encoded}&ctl00%24ContentPlaceHolder1%24hdfMinDateToSelect=27%2F01%2F2017&ctl00%24ContentPlaceHolder1%24hdfMaxDateToSelect={date_encoded}&ctl00%24ContentPlaceHolder1%24btnDescargarZIP=Descargar+ZIP&__EVENTTARGET=&__EVENTARGUMENT=&__LASTFOCUS=&__VIEWSTATE={new_view_state_value}&__VIEWSTATEGENERATOR=94619A0E&__VIEWSTATEENCRYPTED="

        response = session.post(URLMTR, headers=HEADERS, data=conexo_data)
        if response.status_code == 200:
            ...
            if "attachment" in content_disposition and ".zip" in content_disposition:
                ...
                new_filename = f"PRECIOS_CONEXOS_MTR_{sistema}.csv"
    ...
    try:
        ...
        if all_different:
            ...
            date_sin = extract_date("PRECIOS_CONEXOS_MTR_SIN.csv")
            date_bca = extract_date("PRECIOS_CONEXOS_MTR_BCA.csv")
            date_bcs = extract_date("PRECIOS_CONEXOS_MTR_BCS.csv")
            ...
            if date_sin == date_bca == date_bcs:
                ...
                # Set locale and parse the date
                try:
                    if os.name == "nt":
                        locale.setlocale(
                            locale.LC_TIME, "es_ES.UTF-8"
                        )  # Try standard locale names
                    else:
                        locale.setlocale(locale.LC_TIME, "es_ES.utf8")
                    # Parse date like '11/ABR./2025' - Requires Spanish month abbreviations
                    dt = datetime.strptime(
                        date_sin, "%d/%b/%Y"
                    )  # Check if month format needs adjustment (e.g., removing '.')
                    target_date_str_iso = dt.strftime(
                        "%Y-%m-%d"
                    )  # Format as YYYY-MM-DD for API check
                    logging.info(f"Processing data for date: {target_date_str_iso}")
                except Exception as date_err:
                    logging.error(
                        f"Failed to parse date '{date_sin}' or set locale: {date_err}"
                    )
                    delete_csv_files(".")
                    return

                # ===> ADD THE DATE CHECK HERE <===
                data_type_to_check = API_TARGET_SOURCE_CONEXOMTR  # Use the constant defined for this script
                exists = check_date_exists(
                    API_BASE_URL, data_type_to_check, target_date_str_iso
                )
                if exists is None:
                    logging.error(
                        f"Failed to check API for existing data for {target_date_str_iso}. Aborting processing for this date."
                    )
                    delete_csv_files(".")  # Optional: delete if you don't want to retry
                ...
                df_sin = preprocess_csv("PRECIOS_CONEXOS_MTR_SIN.csv", "SIN")
                df_bca = preprocess_csv("PRECIOS_CONEXOS_MTR_BCA.csv", "BCA")
                df_bcs = preprocess_csv("PRECIOS_CONEXOS_MTR_BCS.csv", "BCS")
                ...
                df_sin_actualizado = df_sin_actualizado.rename(
                    columns={
                        "Zona de reserva": "Zona_de_Reserva",
                        " Tipo de reserva": "Tipo_de_reserva",
                        " Precio de la reserva": "Precio_reserva",
                    }
                )
                ...
                for i in range(num_batches):
                    ...
                    batch_success = send_dataframe_to_api(
                        df_batch, API_BASE_URL, API_TARGET_SOURCE_CONEXOMTR
                    )
                    if batch_success:
                ...
                if overall_success and records_successfully_sent_count == num_records:
                    ...
                    message = (
                        f"PND_MTR Script Success: Successfully uploaded {records_successfully_sent_count} "
                    ...
                    # Send Telegram Alert
                    message = (
                        f"PND_MTR Script Error: Date mismatch between files "
            ...
            logging.info(
                "Archivo CSV combinado guardado como PRECIOS_CONEXOS_MTR_actualizado.csv"
            )
        else:
