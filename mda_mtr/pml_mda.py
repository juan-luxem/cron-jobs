...
def get_pml_mda(date_parameter=None):
    ...
    for sistema in SISTEMAS:
        ...
        if match:
            ...
            logging.error("VIEWSTATE not found.")
        ...
        response = session.post(URLMDA, headers=HEADERS, data=marg_data)
    ...
    try:
        ...
        if all_different:
            ...
            print(f"Dates extracted: SIN({date_sin}), BCA({date_bca}), BCS({date_bcs})")
            ...
            if date_sin == date_bca == date_bcs:
                ...
                data_type_to_check = (
                    API_TARGET_SOURCE_PMLMDA  # Use the constant defined for this script
                )
...
#     month = 5
#     year = 2025
#     # Format the day with a leading zero if it's less than 10
#     day_str = f"{day:02d}"
