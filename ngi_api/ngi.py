...
def get_ngi_data():
    try:
        ...
        datafeed_endpoint = (
            f"{ngi_endpoint}forwardDatafeed.json?issue_date={issue_date}"
        )
        ...
        if not response.ok:
            ...
            send_telegram_message(
                bot_token,
                chat_id,
                f"Failed to authenticate with NGI API. Status code: {response.status_code}",
            )
        ...
        if not authToken:
            send_telegram_message(
                bot_token,
                chat_id,
                "Access token not found in NGI authentication response.",
            )
                ...
                gas_data.append(
                    {
                        ...
                        "fechaActualizacion": datetime.now().strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
                    }
                )
        ...
        if response.status_code == 201:
            ...
            send_telegram_message(
                bot_token,
                chat_id,
                "Data successfully processed and saved to the database.",
            )
        else:
            logging.error(
                f"Failed to write data: {response.status_code} - {response.text}"
            )
            send_telegram_message(
                bot_token,
                chat_id,
