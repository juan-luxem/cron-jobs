...
from generacion_ofertada.constants import (
    URL,
    ID_REPORT,
    MIN_DATE,
    VIEW_GENERATOR,
...
def get_generacion_ofertada(market_type: str):
    """
    Generic function to download Ofertas Térmicas data from CENACE
    using requests and BeautifulSoup.

    Flow:
    1. Select Report.
    2. Switch to Hourly (H).
    3. Select System.
    4. Update Period.
    5. Download.
    """
    ...
    for sistema in SISTEMAS:
        ...
        try:
            ...
            # Establish baseline: Report ID, D, SIN
            # ---------------------------------------------------------
            data_report = {
            ...
            # ---------------------------------------------------------
            # STEP 2: Switch to Hourly (H) (Async)
            # Target: ddlPeriodicidad
            # ---------------------------------------------------------
            data_hourly = {
                ...
                "ctl00$ContentPlaceHolder1$ddlSistema": DEFAULT_SYSTEM_ON_LOAD,  # Still SIN
                "ctl00$ContentPlaceHolder1$txtPeriodo": period,
                "ctl00$ContentPlaceHolder1$hdfStartDateSelected": date,
                "ctl00$ContentPlaceHolder1$hdfEndDateSelected": date,
                "ctl00$ContentPlaceHolder1$hdfMinDateToSelect": MIN_DATE[market_type],
            ...
            # ---------------------------------------------------------
            # STEP 3: Select System (Async)
            # Target: ddlSistema. Switch to BCA/BCS if needed.
            # ---------------------------------------------------------
            data_system = {
                ...
                "ctl00$ContentPlaceHolder1$ddlSistema": sistema,  # Switch to Target
                "ctl00$ContentPlaceHolder1$txtPeriodo": period,
                "ctl00$ContentPlaceHolder1$hdfStartDateSelected": date,
                "ctl00$ContentPlaceHolder1$hdfEndDateSelected": date,
                "ctl00$ContentPlaceHolder1$hdfMinDateToSelect": MIN_DATE[market_type],
            ...
            # ---------------------------------------------------------
            # STEP 4: Update Period (Async)
            # Target: txtPeriodo. Confirm dates and params.
            # ---------------------------------------------------------
            data_period = {
