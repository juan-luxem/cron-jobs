...
from servicios_conexos_por_zona_reserva.constants import ID_REPORT, URL, VIEW_GENERATOR


def download_servicios_conexos_por_zona_reserva_files(
    market_type: str,
    ...
    """
    Downloads Servicios Conexos por Zona de Reserva files for MDA.
    """
    download_folder = get_download_folder(start_date=start_date, end_date=end_date)

    if sistema and sistema not in SISTEMAS:
        msg = f"Invalid sistema: {sistema}. Must be one of {SISTEMAS}"
        notify_error(msg)
        return

    if market_type not in URL:
        msg = f"Invalid market_type: {market_type}. Supported types are {list(URL.keys())}"
        notify_error(msg)
        return
    ...
    if start_date and end_date:
        try:
            ...
            msg = f"Date parsing error in servicios conexos por zona reserva: {e}"
            notify_error(msg)
            return
    else:
...
def _process_single_range(market_type, download_folder, fmt_start, fmt_end, sistema):
    try:
        ...
        msg = f"Error initializing session for {market_type} en servicios conexos por zona reserva: {e}"
        notify_error(msg)
        return
    ...
    if fmt_start and fmt_end:
        ...
        target_end = fmt_end
        logging.info(
            f"Downloading Servicios Conexos Zona Reserva {market_type} range: {target_period_str}"
        )
    else:
        ...
        logging.info(
            f"Downloading Servicios Conexos Zona Reserva {market_type} default/current period."
        )
    ...
    try:
        ...
        msg = f"Error selecting report en servicios conexos por zona reserva: {e}"
        notify_error(msg)
        return
    ...
    for sist in sistemas_to_process:
        ...
        try:
            ...
            msg = f"Error switching system to {sist}: {e}"
            notify_error(msg)
            continue
        ...
        try:
            ...
            msg = f"Error refreshing ViewState for {sist}: {e}"
            notify_error(msg)
            continue
        ...
        try:
            ...
            msg = f"Error setting period for {sist}: {e}"
            notify_error(msg)
            continue
        ...
        payload = (
            ...
            f"ctl00%24ContentPlaceHolder1%24ddlSistema={sist}&"
        ...
        try:
            ...
            if r_down.status_code == 200:
                ...
                msg = f"Download failed for {sist}: {r_down.status_code}"
                notify_error(msg)
        except Exception as e:
            msg = f"Error downloading {sist}: {e}"
            notify_error(msg)
