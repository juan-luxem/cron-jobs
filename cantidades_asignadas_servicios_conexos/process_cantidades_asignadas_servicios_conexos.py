...
def process_cantidades_asignadas_servicios_conexos(market_type: str):
    """
    Processes Requerimientos de Servicios Conexos data for the specified market type (MDA or MTR).
    ...
    logging.info(
        f"🚀 Starting Requerimientos de Servicios Conexos {market_type} data processing"
    )
    ...
    if "error" in summary:
        ...
        send_telegram_message(
            bot_token,
            chat_id,
            f"Error en process_cantidades_asignadas_servicios_conexos ({market_type}): {summary['error']}",
        )
    else:
        ...
        if summary["processed"] == summary["total"] and summary["remaining"] == 0:
            ...
            send_telegram_message(
                bot_token,
                chat_id,
                f"Fallo en process_cantidades_asignadas_servicios_conexos ({market_type}): {summary['failed']} de {summary['total']} archivos.",
            )
