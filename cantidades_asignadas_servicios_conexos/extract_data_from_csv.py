    ...
    for _, row in df.iterrows():
        ...
        try:
            record = {
                ...
                'RTReservaRegulacion_MW': float(str(row.get('RTReservaRegulacion_MW', 0)).replace(',', '')),
                'RTReservaRodante_10_MW': float(str(row.get('RTReservaRodante_10_MW', 0)).replace(',', '')),
                'RTReserva_10_MW': float(str(row.get('RTReserva_10_MW', 0)).replace(',', '')),
                'RTReservaSuplementaria_MW': float(str(row.get('RTReservaSuplementaria_MW', 0)).replace(',', '')),
            }
    ...
    if len(csv_files) != 3:
        ...
        if len(csv_files) == 0:
            ...
            send_telegram_message(
                bot_token,
                chat_id,
                f"Error en process_all_csv_files_with_api: No CSV files found in download folder"
            )
        else:
