...
from pml.constants import ID_REPORT, MIN_DATE, URL, VIEW_GENERATOR


def download_pml_files(
    market_type: str,
    ...
    if start_date and end_date:
        # Split user provided range into chunks
        try:
            ranges_to_process = list(chunk_date_range(start_date, end_date))
        except ValueError as e:
            notify_error(f"Date parsing error in PML {market_type}: {e}")
            return
    else:
...
def _process_single_range(
    ...
    try:
        ...
        notify_error(f"Error initializing session for PML {market_type}: {e}")
        return
    ...
    try:
        ...
        notify_error(f"Error extracting initial values for PML {market_type}: {e}")
        return
    ...
    if fmt_start and fmt_end:
        ...
        sel_start = fmt_start
        sel_end = fmt_end
        logging.info(f"Downloading range: {period_str}")
    else:
        period_str = default_period
        sel_start = default_date
        sel_end = default_date
        logging.info("Downloading default/current period.")
    ...
    try:
        ...
        notify_error(f"Error extracting dates for PML {market_type}: {e}")
        return
    ...
    for current_sistema in sistemas_to_process:
        ...
        try:
            ...
            notify_error(
                f"Error switching system to {current_sistema} in PML {market_type}: {e}"
            )
            continue
        ...
        try:
            ...
            notify_error(
                f"Error setting period for {current_sistema} in PML {market_type}: {e}"
            )
            continue
        ...
        try:
            ...
            if r_down.status_code == 200:
                ...
                notify_error(
                    f"Download failed for {current_sistema} in PML {market_type}: HTTP {r_down.status_code}"
                )
        except Exception as e:
            notify_error(
                f"Error downloading {current_sistema} in PML {market_type}: {e}"
            )
