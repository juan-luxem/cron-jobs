    ...
    for _, row in df.iterrows():
        ...
        try:
            record = {
                ...
                'Sistema': sistema,
                'PrecioMarginalLocal_MWh': float(str(row.get('PrecioMarginalLocal_MWh', 0)).replace(',', '')),
                'ComponenteEnergia_MWh': float(str(row.get('ComponenteEnergia_MWh', 0)).replace(',', '')),
                'ComponentePerdidas_MWh': float(str(row.get('ComponentePerdidas_MWh', 0)).replace(',', '')),
                'ComponenteCongestion_MWh': float(str(row.get('ComponenteCongestion_MWh', 0)).replace(',', ''))
            }
